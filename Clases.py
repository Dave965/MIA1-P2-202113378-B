...
formato_partition = "1s 1s 1s i i 16s"
formato_ebr = "1s 1s i i i 16s"
formato_superBlock = "i i i i i 19s 19s i i i i i i i i i i"
formato_inodo = "i i i 19s 19s 19s 15i 1s i"
formato_b_carpeta = "12s i"
formato_b_archivo = "64s"
formato_b_pointers = "16i"

sizeMBR = struct.calcsize(formato_mbr)
sizeEBR = struct.calcsize(formato_ebr)
sizePartition = struct.calcsize(formato_partition)
sizeSuperBlock = struct.calcsize(formato_superBlock)
...
class BloqueArchivos:
    ...
    def deserializar(self,data):
        self.b_content = struct.unpack(formato_b_archivo, data[:64])[0]

    def serializar(self):
        res = struct.pack(formato_b_archivo,  self.b_content)
        return res
...
class SuperBlock:
    ...
    def deserializar(self,data):
        self.s_filesystem_type, self.s_inodes_count, self.s_blocks_count, self.s_free_blocks_count, self.s_free_inodes_count, self.s_mtime, self.s_umtime, self.s_mnt_count, self.s_magic, self.s_inode_s, self.s_block_s, self.s_first_ino, self.s_first_blo, self.s_bm_inode_start, self.s_bm_block_start, self.s_inode_start, self.s_block_start = struct.unpack(formato_superBlock, data[:sizeSuperBlock])

    def serializar(self):
        res = struct.pack(formato_superBlock,  self.s_filesystem_type, self.s_inodes_count, self.s_blocks_count, self.s_free_blocks_count, self.s_free_inodes_count, self.s_mtime, self.s_umtime, self.s_mnt_count, self.s_magic, self.s_inode_s, self.s_block_s, self.s_first_ino, self.s_first_blo, self.s_bm_inode_start, self.s_bm_block_start, self.s_inode_start, self.s_block_start)
        return res

    def graficar(self):
        s="""SB[shape=none label=<
<TABLE border="0" cellspacing="0" cellpadding="10">
<TR><TD border="1" colspan="2" bgcolor="darkgreen" ><FONT color="white">Reporte de <B>SUPERBLOQUE</B></FONT></TD></TR>\n"""
...
class MBR:
    ...
    def __init__(self):
        ...
        self.dsk_fit = b'\0'
        self.mbr_partition = [Partition(),Partition(),Partition(),Partition()]
    ...
    def setAll(self, tamano, fecha, signature, fit):
        self.setFecha(fecha)
    ...
    def deserializar(self,data):
        self.mbr_tamano,  self.mbr_fecha_creacion, self.mbr_dsk_signature, self.dsk_fit = struct.unpack(formato_mbr, data[:sizeMBR])
        for i in range(4):
            self.mbr_partition[i].deserializar(data[sizeMBR+sizePartition*i:])

    def serializar(self):
        res = struct.pack(formato_mbr, self.mbr_tamano,  self.mbr_fecha_creacion, self.mbr_dsk_signature, self.dsk_fit)
...
<TR><TD border="1" colspan="2" bgcolor="indigo" ><FONT color="white"><B>REPORTE DE MBR</B></FONT></TD></TR>\n"""
        s+='<TR><TD>mbr_tamano</TD><TD>'+str(self.mbr_tamano)+'</TD></TR>\n'
        s+='<TR><TD bgcolor="mediumpurple1">mbr_tamano</TD><TD bgcolor="mediumpurple1">'+self.mbr_fecha_creacion.decode()+'</TD></TR>\n'
        s+='<TR><TD>mbr_dsk_signature</TD><TD>'+str(self.mbr_dsk_signature)+'</TD></TR>\n'
        for x in self.mbr_partition:
            if x.part_start == -1:
                continue
            s+=x.imprimir()
            if x.part_type.decode().upper() == "E":
                s+=imprimirExtendida(x,path)
        s+="</TABLE>>];}"
        return s     
    ...
    def showdisk(self,path):
        ...
        espacio_disponible = self.mbr_tamano - sizeMBR - 4*sizePartition
        ...
        for i in range(len(self.mbr_partition)):
            ...
            if x.part_type.decode().upper() == "P":
                s+="<TD> Primaria <BR/><BR/>"+str(round(x.part_s/espacio_disponible*100,2))+"%</TD>"
            elif x.part_type.decode().upper() == "E":
                s+=showExtendida(x,espacio_disponible,path)
                
            inicio = x.part_start+x.part_s

        espacio_libre = self.mbr_tamano-inicio-1
        if espacio_libre>0:
            s+="<TD> Libre <BR/><BR/>"+str(round(espacio_libre/espacio_disponible*100,2))+"%</TD>"
...
class Partition:
    ...
    def setType(self,_type):
        self.part_type = code_str(_type,1)
    ...
    def setAll(self, status, _type, fit, start, s, name):
    ...
    def serializar(self):
        res = struct.pack(formato_partition, self.part_status, self.part_type, self.part_fit, self.part_start, self.part_s, self.part_name)
        return res

    def deserializar(self,data):
        self.part_status, self.part_type, self.part_fit, self.part_start, self.part_s, self.part_name = struct.unpack(formato_partition, data[:sizePartition])

    def imprimir(self):
        s="""<TR><TD border="1" colspan="2" bgcolor="indigo" ><FONT color="white"><B>Particion</B></FONT></TD></TR>\n"""
        s+='<TR><TD>part_status</TD><TD>'+self.part_status.decode().strip("\x00")+'</TD></TR>\n'
        s+='<TR><TD bgcolor="mediumpurple1">part_type</TD><TD bgcolor="mediumpurple1">'+self.part_type.decode().strip("\x00")+'</TD></TR>\n'
        s+='<TR><TD>part_fit</TD><TD>'+self.part_fit.decode().strip("\x00")+'</TD></TR>'
        s+='<TR><TD bgcolor="mediumpurple1">part_start</TD><TD bgcolor="mediumpurple1">'+str(self.part_start)+'</TD></TR>\n'
        s+='<TR><TD>part_s</TD><TD>'+str(self.part_s)+' byte(s)</TD></TR>'
        s+='<TR><TD bgcolor="mediumpurple1">part_name</TD><TD bgcolor="mediumpurple1">'+self.part_name.decode().strip("\x00")+'</TD></TR>\n'
        return s
...
class EBR:
    ...
    def setAll(self, status, fit, start, s, _next, name):
        self.setStatus(status)
        self.setNext(_next)
        self.setFit(fit)
        self.setStart(start)
    ...
    def serializar(self):
        res = struct.pack(formato_ebr, self.part_status, self.part_fit, self.part_start, self.part_s, self.part_next, self.part_name)
        return res

    def deserializar(self,data):
        self.part_status, self.part_fit, self.part_start, self.part_s, self.part_next, self.part_name = struct.unpack(formato_ebr, data[:sizeEBR])

    def imprimir(self):
        s="""<TR><TD border="1" colspan="2" bgcolor="lightcoral" ><FONT color="white"><B>Particion Logica</B></FONT></TD></TR>\n"""
        s+='<TR><TD>part_status</TD><TD>'+self.part_status.decode().strip("\x00")+'</TD></TR>\n'
        s+='<TR><TD bgcolor="lightpink">part_next</TD><TD bgcolor="lightpink">'+str(self.part_next)+'</TD></TR>\n'
        s+='<TR><TD>part_fit</TD><TD>'+self.part_fit.decode().strip("\x00")+'</TD></TR>'
        s+='<TR><TD bgcolor="lightpink">part_start</TD><TD bgcolor="lightpink">'+str(self.part_start)+'</TD></TR>\n'
        s+='<TR><TD>part_s</TD><TD>'+str(self.part_s)+' byte(s)</TD></TR>'
        s+='<TR><TD bgcolor="lightpink">part_name</TD><TD bgcolor="lightpink">'+self.part_name.decode().strip("\x00")+'</TD></TR>\n'
        return s
...
def showExtendida(part, espacio_disponible, path):
    ...
    if ebr.part_status.decode() not in aceptados:
        s+="<TD>"+str(round(part.part_s/espacio_disponible*100,2))+"%</TD>\n"
        colspan+=1
    else:
        inicio = 0
        rangos_ocupados = []
        ...
        for i in range(len(rangos_ocupados)):
            ...
            s+="<TD> EBR </TD>\n"
            s+="<TD> Logica <BR/><BR/>"+str(round(size/espacio_disponible*100,2))+"%</TD>\n"
            colspan+=2
            if i<len(rangos_ocupados)-1:
                size = rangos_ocupados[i+1][0]-rangos_ocupados[i][1]
                size = round(size/espacio_disponible*100,2)
                if size>0:
                    s+="<TD> Libre <BR/><BR/>"+str(size)+"%</TD>\n"
                    colspan+=1
            else:
                size = part.part_s-rangos_ocupados[i][1]
                size = round(size/espacio_disponible*100,2)
                if size>0:
                    s+="<TD> Libre <BR/><BR/>"+str(size)+"%</TD>\n"
                    colspan+=1
                
    s='<TD><TABLE border="1" cellspacing="0" cellpadding="10">\n<TR>\n<TD colspan="'+str(colspan)+'">Extendida</TD>\n</TR>\n<TR>\n'+s+"</TR>\n</TABLE>\n</TD>\n"
    return s    
##print("----prueba Bloque apuntadores-----")
##s = b""
##o = BloqueApuntadores()
##o.b_pointers[5] = 1234
##s += o.serializar()
...
##print(o.__dict__)
##print()
##print("----prueba Bloque archivos-----")
##s = b""
##o = BloqueArchivos()
##o.setB_content("prueba")
##s += o.serializar()
##o.deserializar(s)
##print(o.__dict__)
##print()
##print("----prueba Bloque carpeta-----")
##s = b""
##o = BloqueCarpeta()
##o.b_content[2].setAll("carpeta1", 3)
##s += o.serializar()
##o.deserializar(s)
##for x in o.b_content:
##    print(x.__dict__)
##
##print()
##print("----prueba Inodo-----")
