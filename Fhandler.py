...
def guardar_journaling(comando,params,particion):
    sb = SuperBlock()
    sb.deserializar(particion)
    ...
    inicio = x.decode().strip("\x00")
    pre+= particion[-3450:-(3450-len(inicio))]

    particion = pre+s
    particion = particion[:l].ljust(l,b'\0')
    return particion

def repJournaling(particion):
    sb = SuperBlock()
    sb.deserializar(particion)
    if sb.s_filesystem_type != 3:
        return False, ""
    x = particion[-3450:].decode().strip("\x00")
  ...
  node [fontname="Helvetica,Arial,sans-serif"]
  edge [fontname="Helvetica,Arial,sans-serif"]\n"""
    s += """RJ[shape=none label=<
<TABLE border="0" cellspacing="0" cellpadding="10" bgcolor="plum">
<TR><TD border="1">Comando</TD><TD border="1">Fecha</TD></TR>\n"""

    for fila in filas:
        cols = fila.split("|")
        s+= '<TR>\n<TD border="1">'+cols[0]+'</TD>\n<TD border="1">'+cols[1]+'</TD>\n</TR>\n'
